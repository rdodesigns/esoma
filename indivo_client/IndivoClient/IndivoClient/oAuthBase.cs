...
namespace IndivoClient
{
    public class oAuthBase
    {
        ...
        protected class QueryParameter : IComparable
        {
            ...
            public int CompareTo(object obj)
            {
                if (!(obj is QueryParameter))
                    throw new ArgumentException("Object is not a QueryParameter");

                QueryParameter y = obj as QueryParameter;

                if (this.Name == y.Name)
                {
                    return string.Compare(this.Value, y.Value);
                }
                else
                {
        ...
        public string GenerateSignatureBase(Uri url, string consumerKey, string consumerSecret, string token, string tokenSecret, string userName, string password, string httpMethod, string timeStamp, string nonce, string signatureType, out string normalizedUrl, out string normalizedRequestParameters)
        {
            if (token == null)
            {
                token = string.Empty;
			...
			if (!String.IsNullOrEmpty(password))
			{
                ...
                parameters.Add(new QueryParameter(OAuthPasswordKey, password));
			}
			
            if (!String.IsNullOrEmpty(userName))
                parameters.Add(new QueryParameter(OAuthUsernameKey, userName));
        ...
        /// <param name="httpMethod">The http method used. Must be a valid HTTP method verb (POST,GET,PUT, etc)</param>
        /// <returns>A base64 string of the hash value</returns>
        public string GenerateSignature(Uri url, string consumerKey, string consumerSecret, string token, string tokenSecret, string userName, string password, string httpMethod, string timeStamp, string nonce, out string normalizedUrl, out string normalizedRequestParameters)
        {
            return GenerateSignature(url, consumerKey, consumerSecret, token, tokenSecret, userName, password, httpMethod, timeStamp, nonce, SignatureTypes.HMACSHA1, out normalizedUrl, out normalizedRequestParameters);
        }
